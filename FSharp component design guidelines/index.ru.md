...
* Размеченные объединения(union-types) и записи(records);
...
XML-комментарии на публичном API гарантируют, что у его потребителей будет работать Intellisense и Quickinfo во время использования типов и их членов. Это так же позволяет строить файлы документации для библиотек. Описание всех возможных тегов xml есть в докоментации по языку F#.

    /// Класс представляющий координаты (x,y)
    type Point = 
    
        /// Считает расстояние между этой и другой точкой
        member DistanceTo : other Point:Point -> float
...
## 3. Рекомендации для библиотек, которые будут использоваться только из F#

В этом разделе мы предоставим рекомендации по разработке библиотек, которые будут использоваться только из языка F#. (Рекомендации по внутреннему и внутреннему для сборок(internal) коду описаны в Разделе 4). Есть множество рекомендации относительно проектирования библиотек, примениемые именно для F#, но в отсуствии оных следуюет всегда обращаться к документу .NET Library Design Guidelines.

### 3.1 Соглашения по наименованию

#### ✔ Используйте соглашения .NET по наименованию и капитализации для всего объектно-ориентированного кода, даже где потребителем библиотеки будет только F#

Таблица 1. Соглашения, связанные с публичными конструкциями платформы .NET Framework, а также соглашения для F#-F# библиотек.

| Конструкция | Соглашение по наименованию | Часть речи | Пример | Замечания |  
|-----------|------|------|----------|-------|
| Конкретный тип | PascalCase | Существительное/ наречие | List, Double, Complex | Под конкретным типом подразумевается тип структуры, класса, перечисления, делегата, записи и размеченного объединения. Не смотря на то что традиционно в OCaml наименования типов были в нижнем регистре, в F# приняты соглашения по наименованию .NET. 
| Сборки          | PascalCase |                 | Fabrikom.Core.dll |  |
| Варианты размеченного объединения     | PascalCase | Существительное | Some, Add, Success | Не используйте префиксы в публичном API. Разрешается использоваться префиксы для внутреннего API, например: `type Teams = TAlpha | TBeta | TDelta.` |
| События          | PascalCase | Глагол | ValueChanged |  |
| Исключения     | PascalCase |      | WebException | В конце наименования должно быть “Exception”. |
| Поля          | PascalCase | Существительное | CurrentName  | |
| Типы интерфейсов |  PascalCase | Существительное/ наречие | IDisposable | Наименование должно начинаться с “I”. |
| Метод |  PascalCase |  Глагол | ToString | | 
| Пространство имен | PascalCase | | Microsoft.FSharp.Core | Обчно используют подход <Организация>.<Наименование проекта>[.<Подпространства имен>], но наименование организации можно не писать, если проект не имеет за собой организации.  |
| Параметры | camelCase | Существительное |  typeName, transform, range | | 
| let-значения (внутренние) | camelCase или PascalCase | Существительное/ глагол |  getValue, myTable |
| let-значения (внешние) | camelCase или PascalCase | Существительное/ глагол  | List.map, Dates.Today | let-связанные значения часто доступны в публичном API, когда разработка осуществляется согласно традиционным подходам функционального программирования. Однако, лучше использовать формат наименования PascalCase, когда данный API планируется использовать из других языков .NET. |
| Свойства  | PascalCase  | Существительное/ наречие  | IsEndOfFile, BackColor  | Boolean-свойства обычно содержат префикс Is или Can в наименовании. Наименование также должность нести положительный утвердительный смысл, например `IsEndOfFile`, а не `IsNotEndOfFile`.

Таблица 1 суммирует общие рекомендации по наименованию и капитализации в коде. У нас так же имеется ряд уточнений для них относительно некоторых конструкций F#.
Будте знакомы со следующими принципами:
